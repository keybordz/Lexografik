        ...
        lexStats.dateString =
          ...
          // Error case header
            ...
            if (!lexStats.dictionary.includes(newWord)) {
              ...
              let thirdLast = newWord.substring(wordLength - 3, wordLength - 2);
              ...
              } else if ( (lastTwo === "LY" && thirdLast !== "L") ||
                          (lastTwo === "ED" && thirdLast !== "E") ||
                          (lastTwo === "ER" && thirdLast !== "E") ||
                          (lastTwo === "OR" && thirdLast !== "O") ) {
              ...
              // Plurals and other Y enders
                ...
                // Special case: 4-letter S Plurals
                if (lastChar === "S" && wordLength === 4) {
                  lexStats.endCounter["S4"] += 1;
                }
              }
...
dictData = JSON.stringify(lexStats)
const filePath = '/Users/jaywright/Sites/dictionary.json';
fs.writeFile(filePath, dictData, (error) => { console.log(error) })
...
const server = http.createServer( (request, response) => {
  ...
  html += `${pluralCount} plurals: `
  html += `${plural5Count} non-unique (${pluralPct}%) / `;
  html += `${plural4Count} unique<br>`
  ...
  const rowSize = 15;     // Number of words per row
  var rowIndex = 0;
  ...
  _.each(letters, (letter) => {
      html += `<h3>${letter} - ${lexStats.startCounter[letter]} words</h3>`;
      ...
      // Close out the table for this section
      html += `</table>`;
      rowIndex = 0;
  });
